...
uint32_t FST::valueMemU() { return static_cast<uint32_t >(upper_values.bit_size() / 8); }
...
void FST::load(vector<uint8_t> &keys, vector<uint64_t> &values, int longestKeyLen) {
    ...
    upper_values.resize(val_posU);
    for (auto i = 0; i < val_posU; i++) {
        upper_values[i] = ta_to_code[valuesU_[i]];
    }

    val_memU_ = static_cast<uint32_t >(upper_values.bit_size() / 8);
